...
- **Edge aggregation**: all streams between the same pair of systems collapse
  ...
  A legend sits in the corner and is included in exports.
...
- **Filtering**: a text search (matches system, stream, entity, and field
  names) plus status / timing / business-need facets. The canvas and the
...
`flows.json`, and `layout.json`. Writes are atomic (temp file + rename) and guarded by a
...
GET                /api/projects/{proj}/graph        # everything at once, incl. layout
...
A headless browser smoke test for the UI lives in `web/js/e2e.js`; open
`http://127.0.0.1:8484/?e2e#/p/YourProject` (or drive it with headless Chrome
and `--dump-dom`) and read the `#e2e-results` block.
